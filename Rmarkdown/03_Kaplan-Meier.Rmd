...
<br/><br/>
Om de 'survival' package in vier dagen te leren ga ik de volgende doen. Ten eerste ga ik papers en workflows opzoeken over de package. Met de informatie kan ik kennis vergroten en opzoeken wat ik nodig heb in de datasets. Een paper die ik ga bestuderen is bijvoorbeeld /////////////(Rich e.a.). Ik ga op de internet nepdata of bestaande data opzoeken die gerelateerd is op onderzoeken van ziektes met overlevingskans of periode van overlijding. Ook ga ik op YouTube video's zoeken met uitleg over de gebruik van de package. 
...
# view tabel
library(reactable)
reactable(heart_data, defaultPageSize =5, compact = TRUE)
```
...
```{r echo=F, message=F, fig.cap= "Kaplan-Meier plot van de data Heart Failure Prediction gebaseerd op geslacht, waarbij de tijd in dagen op de x-as staat, de overlevingskans op de y-as. Verschillende symbolen vertegenwoordigen verschillende experimenttypes."}
...
```{r echo=F, message=F, fig.cap= "Kaplan-Meier plot van de data Heart Failure Prediction gebaseerd op leeftijd, waarbij de tijd in dagen op de x-as staat, de overlevingskans op de y-as. Verschillende symbolen vertegenwoordigen verschillende experimenttypes."}
